...
if (length(kf) > 0) {
  kmers_long <- map_dfr(kf, read_fasta) |> filter(seq != "") |>
    ...
    filter(!is.na(kidx)) |> arrange(seq_name, kidx) |>
    distinct(seq_name, kidx, .keep_all = TRUE) |> mutate(kcol = paste0("kmer", kidx))
  KMER_COLS <- paste0("kmer", seq_len(max(kmers_long$kidx)))
