...
contigs_s <- if (length(ff) > 0)
  map_dfr(ff, ~ read_fasta(.x) |> mutate(sample = str_extract(basename(.x), "JB_[0-9]+"))) |>
...
kmers_s <- if (length(kf) > 0)
  map_dfr(kf, ~ read_fasta(.x) |> mutate(sample = str_extract(.x, "JB_[0-9]+"))) |>
    filter(seq != "") |>
