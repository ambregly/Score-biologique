...
# Contigs (JB_*.fasta) & k-mers (JB_*_kmers/kmers.fa), rattachés par seq_name
ff <- list.files(params$dir_fasta, pattern = "^JB_.*\\.fasta$", full.names = TRUE)
if (length(ff) > 0) {
  contigs <- map_dfr(ff, read_fasta) |> filter(seq != "") |>
    distinct(id, .keep_all = TRUE) |> transmute(seq_name = id, contig_seq = seq)
  annot <- annot |> left_join(contigs, by = "seq_name")
} else annot$contig_seq <- NA_character_
kf <- list.files(params$dir_kmers, pattern = "^kmers\\.fa$", recursive = TRUE, full.names = TRUE)
if (length(kf) > 0) {
  kmers_long <- map_dfr(kf, read_fasta) |> filter(seq != "") |>
    mutate(seq_name = sub("\\.kmer.*$", "", id)) |> distinct(seq_name, seq) |>
    group_by(seq_name) |> mutate(kcol = paste0("kmer", row_number())) |> ungroup()
  KMER_COLS <- paste0("kmer", seq_len(max(as.integer(sub("kmer", "", kmers_long$kcol)))))
  kmers_wide <- kmers_long |>
    pivot_wider(id_cols = seq_name, names_from = kcol, values_from = seq) |>
    select(seq_name, all_of(KMER_COLS))
  annot <- annot |>
    left_join(count(kmers_long, seq_name, name = "n_kmers"), by = "seq_name") |>
